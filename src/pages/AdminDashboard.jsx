...
import {
  ALL_WAGONS, ZONE_STATS, ZONE_CITIES, ZONE_LINE_DATA,
...
const AdminDashboard = () => {
  ...
  const stats      = ZONE_STATS[zone]      || ZONE_STATS.NR;
  const wagons     = ALL_WAGONS.filter(w => w.zone === zone).slice(0, 8);
  const cities     = ZONE_CITIES[zone]     || [];
  const lineData   = ZONE_LINE_DATA[zone]  || [];
  const barData    = ZONE_BAR_DATA[zone]   || [];
  ...
  // Pie data derived from zone alerts
  const pieData = [
    { name:"GPS Alert",   value: alerts.filter(a=>a.type.includes("GPS")).length   || 1 },
    { name:"Route Dev.",  value: alerts.filter(a=>a.type.includes("Route")).length || 1 },
    { name:"Maintenance", value: stats.maint },
    { name:"Cargo Alert", value: alerts.filter(a=>a.type.includes("Cargo")).length || 1 },
  ];
  ...
  return (
    <DashboardLayout>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, padding:"12px 18px", background:"rgba(37,99,235,.08)", border:"1px solid rgba(59,130,246,.2)", borderRadius:12 }}>
        <div>
          <span style={{ color:"#f1f5f9", fontWeight:700, fontSize:15 }}>Zone {zone} — {admin?.region}</span>
      ...
      <div style={{ display:"flex", gap:"14px", marginBottom:"20px", flexWrap:"wrap" }}>
        <StatCard title="Total Wagons"     value={stats.total}              color="#3b82f6" icon={FiTruck}        trend="+3.2%"  trendUp />
        <StatCard title="Active Wagons"    value={stats.active}             color="#22c55e" icon={FiActivity}     trend="+1.8%"  trendUp />
        <StatCard title="Delayed Wagons"   value={stats.delayed}            color="#f59e0b" icon={FiAlertTriangle} trend="-5.1%" trendUp={false} />
        <StatCard title="Maintenance Req." value={stats.maint}              color="#ef4444" icon={FiTool}          trend="+2"    trendUp={false} />
        <StatCard title="Stations Online"  value={stats.stations}           color="#8b5cf6" icon={FiMapPin}        trend="+1"    trendUp />
        <StatCard title="GPS Active"       value={stats.gps}                color="#06b6d4" icon={FiWifi}          trend="+0.9%" trendUp />
        <StatCard title="AI Alerts"        value={stats.alerts}             color="#f97316" icon={FiBell}          trend="-12%"  trendUp />
        <StatCard title="Cargo Loads"      value={stats.cargo}              color="#10b981" icon={FiBox}           trend="+6.4%" trendUp />
      </div>

      {/* Wagon Activity Table */}
      <div className="card mb-20">
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"16px" }}>
          <div className="section-title" style={{ margin:0 }}>Recent Wagon Activity — Zone {zone}</div>
          <button className="btn btn-ghost btn-sm" onClick={() => navigate("/wagons")}>View All</button>
        </div>
        <div className="table-wrap">
          <table>
            <thead>
              <tr><th>Wagon ID</th><th>Current Location</th><th>Destination</th><th>Speed</th><th>Status</th></tr>
            </thead>
            <tbody>
              {wagons.map(w => (
                <tr key={w.id}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 320px", gap:"20px", marginBottom:"20px" }}>
        <div className="card">
          <div className="section-title">Wagon Movement Analytics — Zone {zone}</div>
        ...
        <div className="card">
          <div className="section-title">System Health — Zone {zone}</div>
          {health.map(h => (
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 280px", gap:"20px", marginBottom:"20px" }}>
        <div className="card">
          <div className="section-title">Station Activity — Zone {zone}</div>
      ...
      <div className="card mb-20">
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"16px" }}>
          <div className="section-title" style={{ margin:0 }}>Live Tracking — Zone {zone} Hubs</div>
          ...
          {cities.map(c => (
            <div key={c.name} style={{ background:"#071628", border:"1px solid #1a3356", borderRadius:"12px", padding:"16px" }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"6px" }}>
                <div style={{ fontSize:"11px", color:"#64748b" }}>Active Routes</div>  <div style={{ fontSize:"12px", color:"#22c55e", fontWeight:700 }}>{c.active}</div>
                <div style={{ fontSize:"11px", color:"#64748b" }}>Moving Wagons</div> <div style={{ fontSize:"12px", color:"#3b82f6", fontWeight:700 }}>{c.moving}</div>
                <div style={{ fontSize:"11px", color:"#64748b" }}>Delayed Routes</div><div style={{ fontSize:"12px", color:"#f59e0b", fontWeight:700 }}>{c.delayed}</div>
                <div style={{ fontSize:"11px", color:"#64748b" }}>Offline GPS</div>   <div style={{ fontSize:"12px", color:"#ef4444", fontWeight:700 }}>{c.offline}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="card mb-20">
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"16px" }}>
          <div className="section-title" style={{ margin:0 }}>AI Alerts — Zone {zone}</div>
      ...
      {/* Predictive Insights */}
      <div className="card mb-20">
        <div className="section-title">Predictive Insights — Zone {zone}</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 300px", gap:"20px" }}>
        <div className="card">
          <div className="section-title">Recent System Logs — Zone {zone}</div>
