              ...
              <div style={{ position:"absolute", right:0, marginTop:8, width:320, background:"var(--dropdown-bg,#0d1f3c)", border:"1px solid var(--border-color,#1a3356)", borderRadius:14, overflow:"hidden", zIndex:500, boxShadow:"0 16px 48px rgba(0,0,0,.3)" }}>
                ...
                {notifs.length === 0 ? (
                  ...
                  notifs.map(n => {
                    ...
                    return (
                      <div key={n.id}
                        style={{ padding:"11px 16px", borderBottom:"1px solid var(--border-color)", display:"flex", alignItems:"center", gap:10, background: isRead ? "transparent" : "rgba(37,99,235,.05)" }}
          ...
          <div ref={profileRef} style={{ position:"relative" }}>
            ...
            >
              ...
              <div style={{ textAlign:"left" }}>
                ...
                <div style={{ color:"var(--text-muted,#4a6fa5)", fontSize:10 }}>Zone {zone} &middot; Operator</div>
              </div>
            ...
            {showProfile && (
              <div style={{ position:"absolute", right:0, marginTop:8, width:220, background:"var(--dropdown-bg,#0d1f3c)", border:"1px solid var(--border-color,#1a3356)", borderRadius:14, overflow:"hidden", zIndex:500, boxShadow:"0 16px 48px rgba(0,0,0,.3)" }}>
                <div style={{ padding:"12px 16px", borderBottom:"1px solid var(--border-color,#1a3356)" }}>
                  ...
                  <div style={{ color:"var(--text-muted,#4a6fa5)", fontSize:11, marginTop:2 }}>{operator?.email}</div>
                ...
                {[
                  { label:"Profile",  Icon:FiUser,     path:"/operator/settings?tab=Profile"  },
                  { label:"Settings", Icon:FiSettings, path:"/operator/settings?tab=Theme"    },
                ].map(({ label, Icon, path }) => (
