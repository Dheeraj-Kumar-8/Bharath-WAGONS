...
const StatCard = ({ title, value, color = "#3b82f6", icon: Icon, trend, trendUp = true, sub }) => (
  <div style={{
    background: "#0d1f3c", border: `1px solid ${color}28`,
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px" }}>
      <span style={{ color: "#64748b", fontSize: "12px", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".5px" }}>{title}</span>
      {Icon && (
    ...
    <div style={{ color: "#f1f5f9", fontSize: "28px", fontWeight: 800, lineHeight: 1, marginBottom: "8px" }}>{value}</div>
    {sub && <div style={{ color: "#64748b", fontSize: "12px", marginBottom: "8px" }}>{sub}</div>}
    {trend && (
      <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
        ...
        <span style={{ color: "#4a6fa5", fontSize: "11px" }}>vs last week</span>
      </div>
    )}
  </div>
);
