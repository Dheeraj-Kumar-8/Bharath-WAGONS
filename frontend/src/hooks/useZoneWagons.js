...
export function useZoneWagons(options = {}) {
  ...
  const zoneWagons = useMemo(() => (
    zone ? allWagons.filter((wagon) => wagon.zone === zone) : allWagons
  ), [allWagons, zone]);

  const datedWagons = useMemo(() => (
    filterWagonsByDateRange(zoneWagons, dateFrom, dateTo)
  ), [zoneWagons, dateFrom, dateTo]);
