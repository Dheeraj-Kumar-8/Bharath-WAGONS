...
const INIT_NOTIFS = [
  { id: 1, color: "#ef4444", title: "Critical: NR Zone Delay Spike",  sub: "18 wagons delayed · 3 min ago",  badge: "Critical" },
  { id: 2, color: "#f59e0b", title: "Monthly Report Ready",            sub: "June 2025 analytics ready",       badge: "Info"     },
  { id: 3, color: "#3b82f6", title: "On-Time Rate Improved",           sub: "SR Zone +2.1% this week",         badge: "Good"     },
];
...
// ── Inner layout that consumes AnalyticsSearchContext ─────────────────────────
const AnalyticsLayoutInner = ({ children, title, sub }) => {
  ...
  // Ctrl+K opens analytics search modal
  useEffect(() => {
    const h = e => {
    ...
    <div className="page-wrapper">
      ...
      <div style={{
        ...
        <nav style={{ flex: 1, overflowY: "auto", padding: "14px 10px" }}>
          <div style={{ color: "var(--nav-label,#1e3a5f)", fontSize: "10px", fontWeight: 700, letterSpacing: "2px", marginBottom: "10px", paddingLeft: "8px" }}>ANALYTICS</div>
      ...
      <div className="main-area">
        ...
        <div style={{
          ...
          <div style={{ flexShrink: 0 }}>
            <div style={{ color: "var(--text-strong,#f1f5f9)", fontWeight: 700, fontSize: "16px", lineHeight: 1.2 }}>Analytics Dashboard</div>
            <div style={{ color: "#a855f7", fontSize: "11px", fontWeight: 500 }}>
              {analyst?.region || "Analytics Center"} &mdash; Zone {analyst?.zone}
            </div>
          </div>

          {/* Center: Search trigger — opens full AnalyticsSearchModal */}
          <div
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
            ...
            <div ref={notifRef} style={{ position: "relative" }}>
              ...
              {showNotif && (
                <div style={{ position: "absolute", right: 0, marginTop: 8, width: 300, background: "var(--dropdown-bg,#0d1f3c)", border: "1px solid var(--border-color,#1a3356)", borderRadius: 14, overflow: "hidden", zIndex: 500, boxShadow: "0 16px 48px rgba(0,0,0,.3)" }}>
                  <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border-color,#1a3356)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ color: "var(--text-strong,#f1f5f9)", fontWeight: 700, fontSize: 14 }}>Notifications</span>
                    {unread > 0 && <button onClick={markAll} style={{ background: "none", border: "none", color: "#a855f7", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Mark all read</button>}
                  </div>
                  {notifs.length === 0
                    ? <div style={{ padding: "20px 16px", textAlign: "center", color: "#4a6fa5", fontSize: 13 }}>✓ All caught up!</div>
                    : notifs.map(n => {
                      const read = readIds.has(n.id);
                      return (
                        <div key={n.id} style={{ padding: "11px 16px", borderBottom: "1px solid var(--border-color)", display: "flex", gap: 10, alignItems: "center", background: read ? "transparent" : "rgba(168,85,247,.05)" }}>
                          <div style={{ width: 6, height: 6, borderRadius: "50%", background: read ? "transparent" : n.color, flexShrink: 0 }} />
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ color: read ? "var(--text-muted)" : "var(--text-strong)", fontSize: 12, fontWeight: read ? 400 : 600 }}>{n.title}</div>
                            <div style={{ color: "var(--text-muted)", fontSize: 11 }}>{n.sub}</div>
                          </div>
                          <div style={{ display: "flex", gap: 4 }}>
                            {!read && <button onClick={() => markRead(n.id)} style={{ background: "rgba(34,197,94,.1)", border: "1px solid rgba(34,197,94,.25)", borderRadius: 5, padding: "2px 6px", color: "#22c55e", fontSize: 10, cursor: "pointer" }}>✓</button>}
                            <button onClick={() => dismiss(n.id)} style={{ background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.2)", borderRadius: 5, padding: "2px 6px", color: "#ef4444", fontSize: 10, cursor: "pointer" }}>✕</button>
                          </div>
                        </div>
                      );
                    })
            ...
            <div ref={profileRef} style={{ position: "relative" }}>
              <button onClick={() => { setShowProfile(p => !p); setShowNotif(false); }}
                ...
                <div style={{ textAlign: "left" }}>
                  <div style={{ color: "var(--text-strong,#f1f5f9)", fontSize: 12, fontWeight: 600 }}>{analyst?.name || "Analytics User"}</div>
                  <div style={{ color: "var(--text-muted,#4a6fa5)", fontSize: 10 }}>Zone {analyst?.zone} · Analytics</div>
                </div>
              ...
              {showProfile && (
                <div style={{ position: "absolute", right: 0, marginTop: 8, width: 220, background: "var(--dropdown-bg,#0d1f3c)", border: "1px solid var(--border-color,#1a3356)", borderRadius: 14, overflow: "hidden", zIndex: 500, boxShadow: "0 16px 48px rgba(0,0,0,.3)" }}>
                  <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border-color,#1a3356)" }}>
                    <div style={{ color: "var(--text-strong,#f1f5f9)", fontWeight: 700, fontSize: 13 }}>{analyst?.name}</div>
                    ...
                    <div style={{ marginTop: 6 }}>
                      ...
                      <span style={{ color: "#a855f7", fontSize: 11, marginLeft: 6, fontWeight: 600 }}>Analytics</span>
                    </div>
                  </div>
                  {[{ label: "Profile", Icon: FiUser, path: "/analytics-dashboard/settings?tab=Profile" }, { label: "Settings", Icon: FiSettings, path: "/analytics-dashboard/settings?tab=Theme" }].map(({ label, Icon, path }) => (
...
// ── Wrap with provider so useAnalyticsSearch() works inside ──────────────────
